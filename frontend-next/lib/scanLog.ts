...
import { isPersistedImageUrl, normalizeImageUrl as normalizePersistedImage, compressDataUrlForSync } from "@/lib/persistImage";
...
function normalizeImageUrl(url?: string): string {
  return normalizePersistedImage(url);
}
...
export async function syncScanLogsFromServer(): Promise<ScanLog[]> {
  ...
  try {
    ...
    const merged = mergeScanLogs(loadScanLogs(), apiHistory);
...
export async function syncScanLogToServer(log: ScanLog): Promise<void> {
  ...
  try {
    const imageForSync = log.imageUrl && isPersistedImageUrl(log.imageUrl)
      ? await compressDataUrlForSync(log.imageUrl)
      : "";
    await api.updatePrediction(log.backendId, {
      image_data: imageForSync,
    ...
    // Thumbnail is already saved at predict time; reports/plant sync can retry later
  }
}
